...
class VisionHVPSolver(AutogradHVPSolver):
    ...
    def get_losses(self) -> List[Tensor]:
        ...
        # hat_rep, hat_s, hat_y = self.network(inputs)
        hat_rep, hat_s, hat_y = self.network(features, T)

        # print(hat_s.shape, S.shape, hat_y.shape, Y.shape)
        loss_s = self.criterion(hat_s, S)
        loss_y = self.criterion(hat_y, Y)
        net_loss = self.mi_estimator(hat_rep, T)

        return [loss_s, loss_y, net_loss]
